...
def register(request):
    ...
    if(request.method == 'POST'):
        ...
        # store_name=profile_form['StoreName']
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # One to one relationship indicator(Will research into it in some moments)
            profile.user = user
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # gets the username and password from the form supplied
        password = request.POST['password']

        # authenticates the user in a single line of code, Hail Django!
        user = authenticate(request, username=username, password=password)
        print("user:", user)
        if user:
            if user.is_active:
                login(request, user)
                # if user has logged in successfully, he/she would be redirected to the index page
